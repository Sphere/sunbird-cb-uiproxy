...
sunbirdrRcCertificate.post('/events', async (req, res) => {
  ...
  // tslint:disable-next-line: max-line-length
  const query =
    'INSERT INTO sunbird.rc_events (eventId, eventName, eventDescription, eventDate, eventPlace, eventType, createdBy, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
  // tslint:disable-next-line: max-line-length
  const params = [
    eventId,
    eventName,
...
sunbirdrRcCertificate.post('/events/edit', async (req, res) => {
  ...
  try {
    const result = await client.execute(getEventQuery, [eventId], {
      prepare: true,
    });
...
sunbirdrRcCertificate.get('/events/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const eventData = await client.execute(getEventQuery, [id], {
      prepare: true,
    });
...
sunbirdrRcCertificate.post('/events/users', async (req, res) => {
  try {
    ...
    if (!Array.isArray(users) || users.length === 0) {
      return res
        .status(400)
        .json({ error: 'Please provide a list of users to link to the event' });
    }
...
async function insertUserEventLink(queryParamsLink: any) {
  // tslint:disable-next-line: max-line-length
  const queryLink =
    'INSERT INTO sunbird.rc_events_users (linkid, userid, eventid, firstname, lastname, place, certificateGenerationStatus, createdat, updatedat) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
async function tryGenerateCertificateWithRetry(
  user,
  eventDetails,
  templateId,
  retries = 2
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const result = await generateCertificateFromRcMapper(
        user,
        eventDetails,
        user.userid,
        templateId
      ...
      logInfo(
        `Attempt ${attempt} failed for user ${user.userid}: ${e.message}`
      );
    }
    await delay(200);
  }
...
sunbirdrRcCertificate.post('/events/generateCertificates', async (req, res) => {
  try {
    ...
    const eventStatusUpdate = ['inProgress', new Date(), eventId];
    await updateEventStatus(eventStatusUpdate);

    res
      .status(200)
      .json({ message: 'Certificate generation started', eventId });
    ...
    for (const user of users) {
      ...
      if (
        !userid ||
        user.certificateGenerationStatus === 'failed during user creation'
      ) {
        failedCount++;
        continue;
      }
      ...
      try {
        const certificateGenerationStatus =
          await tryGenerateCertificateWithRetry(user, eventDetails, templateId);
        const status = certificateGenerationStatus
          ? 'success'
          : 'failed during certificate generation';
        const queryParams = [
          status,
          new Date(),
          templateId,
        ...
        logInfo(
          `Error generating certificate for userId ${userid}: ${JSON.stringify(
            err
          )}`
        );
        failedCount++;
    ...
    logInfo(
      `Certificate generation job ${eventId} finished: ${successCount} success, ${failedCount} failed`
    );
  } catch (error) {
  ...
  try {
    ...
    if (userSearch.data.result.response.count > 0) {
      ...
      return {
        ...
        lastName:
          userData.profileDetails.profileReq.personalDetails.surname ||
          userData.profileDetails.profileReq.personalDetails.firstname,
        status: true,
        userId: userData.id,
      };
    }
...
sunbirdrRcCertificate.get(
  '/downloadCertificates/:eventId',
  async (req, res) => {
    try {
      const { eventId } = req.params;
      const listParams = {
      ...
      for (const key of fileKeys) {
        ...
        const fileStream = s3
          .getObject({ Bucket: listParams.Bucket, Key: key })
          .createReadStream();
        zipStream.append(fileStream, {
          name: key.replace(listParams.Prefix, ''),
        }); // Strip the folder path from the file name
      }
      zipStream.finalize();
    } catch (err) {
      logInfo(JSON.stringify(err));
