...
export const learnerPathApiV2 = Router()
...
learnerPathApiV2.post('/', async (req, res) => {
  try {
    logInfo('***********  learner path v2 post')
    logInfo('Inside learner path v2 api (portal)', JSON.stringify(req.body))
...
learnerPathApiV2.get('/', async (req, res) => {
  try {
    logInfo('***********  learner path v2')
    const userId = req.query.userId as string
    logInfo('Inside learner path v2 api (portal)', JSON.stringify(userId))
