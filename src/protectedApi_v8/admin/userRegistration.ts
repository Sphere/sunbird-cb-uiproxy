  ...
  try {
    ...
    clientConnect.execute(query, params, (err, _result) => {
      if (!err) {
        ...
        res
          .status(400)
          .send('/user/update-access-path:: Something went wrong!')
      }
    })
    // })
  } catch (err) {
...
userRegistrationApi.post('/bulkUpload', async (req, res) => {
  try {
    ...
    res.json(
      `User upload started with id: ${uuid}, After process is finished you can download the report`
    )
        ...
        for (const row of sheet.data.slice(1)) {
          if (row.length) {
            ...
            if (!validFname || !validLname || !validEmail) {
              ...
              const userId = await createUser(reqToNewUser).catch((err) => {
                if (err.response.status === 409) {
                  ...
                  reportData.push([
                    `\n${email}`,
                    `User could not be created in Keycloack`,
                  ])
                }
              })
              if (userId) {
                let msg = ''
                await performNewUserSteps(
                  userId,
                  req,
                  reqToNewUser.body.email,
                  yesRoles
                ).catch((err) => {
                  // reportData.push([`\n${email}`, `${err}`])
                  msg = `${err}`
...
userRegistrationApi.get('/bulkUploadData', async (req, res) => {
  try {
    const clientConnect = new cassandraDriver.Client(cassandraClientOptions)
    const query = `SELECT id,name,status FROM ${
      CONSTANTS.CASSANDRA_KEYSPACE
    }.bulk_user_upload_detail
