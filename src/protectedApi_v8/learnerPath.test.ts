/**
 * learnerPath.ts — two routes, both using the callable `axios({...})` form.
 *
 ...
 * present, so this is a safe single-response validation branch, unlike the
 * no-`return` shape flagged elsewhere in this codebase.
 *
 ...
 * sibling catalog.test.ts / assessment.test.ts approach.
 */
...
const agent = () => mountRouter(learnerPathApi)
