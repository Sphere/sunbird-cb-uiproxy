...
// CDN URL replacement handler for direct content/v1/read endpoint (direct API call)
// tslint:disable-next-line: no-any
mobileAppApi.get('/content/v1/read/*', async (req, res) => {
  try {
    const contentId = req.originalUrl.split('/').pop()
    logInfo('[Content Mobile API] Intercepting direct content/v1/read for ID:', contentId)
