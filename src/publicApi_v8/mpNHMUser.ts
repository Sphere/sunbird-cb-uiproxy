...
// sonar-cleanup: local getUserDetails/createUser/assignRoleToUser/OTP-axios-calls/migrateUserToMp replaced with shared imports (CHANGE 30)
import {
...
const serviceSchemaJoi = Joi.object({
    ...
    blockOthers: Joi.string()
        ...
        .messages({
            [ANY_REQUIRED_KEY]: 'Block name is required when block is set to Other',
        }),
    ...
    facilityNameOthers: Joi.string()
        ...
        .messages({
            [ANY_REQUIRED_KEY]: 'Facility name is required when facility name is set to Others',
        }),
...
const createUser = async (userDetails: UserDetails) =>
    createOrgSignupUser(userDetails, 'MP', (details) => getDetailsAsPerRole(details).orgName, 60000)

const assignRoleToUser = async (userId: string, userDetails: UserDetails) =>
    assignOrgSignupUserRole(userId, userDetails, (details) => getDetailsAsPerRole(details).orgId, 60000)
// tslint:disable-next-line: all
