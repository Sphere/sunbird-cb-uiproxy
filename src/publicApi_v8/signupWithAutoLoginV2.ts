...
signupWithAutoLoginV2.post('/register', async (req, res) => {
  try {
    ...
    try {
      ...
      req.session.user_session_data = profileData
      req.session.user_session_data.userId = userId
...
signupWithAutoLoginV2.post('/validateOtpWithLogin', async (req: any, res) => {
  try {
    ...
    const mobileNumber = req.session.user_session_data.phone || ''
    const email = req.session.user_session_data.email || ''
    const validOtp = req.body.otp
    const userUUId = req.session.user_session_data.userId
    const password = req.session.user_session_data.password
    if (!validOtp) {
